...
function CartList(){
    const {listItemsIntoCart, addToCart, removeToCart} = useCart()
    const totalItems = listItemsIntoCart?.reduce((acc, item) => {
        return [acc[0] + item.times, acc[1] + (item.price * item.times)]
    },[0, 0])
            ...
            <div className="w-full max-w-7xl px-4 md:px-5 lg-6 mx-auto relative z-10">
                <div className="grid grid-cols-12">
                    <div
                        ...
                        {listItemsIntoCart?.map( item => (
                            <div key={item.id} className="flex flex-col min-[500px]:flex-row min-[500px]:items-center gap-5 py-6  border-b border-gray-200">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-4 w-full">
                                    <div className="md:col-span-2">
                                        <div className="flex flex-col max-[500px]:items-center gap-3">
                                            ...
                                            <h6 className="font-normal text-base leading-7 text-gray-500">{item.category.name}</h6>
                                            <h6 className="font-semibold text-base leading-7 text-indigo-600">${item.price}</h6>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center max-[500px]:justify-center md:justify-end max-md:mt-3 h-full">
                                        <p className="font-bold text-lg leading-8 text-indigo-600 text-center">${item.price * item.times}</p>
                                    </div>
                                </div>
                            </div>
                        ))}
                        <div className="flex items-center justify-end mt-8">
                            <button
                                className="flex items-center px-5 py-3 rounded-full gap-2 border-none outline-0 font-semibold text-lg leading-8 text-indigo-600 shadow-sm shadow-transparent transition-all duration-500 hover:shadow-indigo-300 hover:bg-indigo-50">
                                Agregar un cupón
                    ...
                    <div
                        ...
                        <h2 className="font-manrope font-bold text-3xl leading-10 text-black pb-8 border-b border-gray-300">
                            Resumen de su orden</h2>
                        <div className="mt-8">
                            <div className="flex items-center justify-between pb-6">
                                <p className="font-normal text-lg leading-8 text-black">{totalItems[0]} Items</p>
                                <p className="font-medium text-lg leading-8 text-black">${totalItems[1]}</p>
                            </div>
                            <form>
                                ...
                                <div className="flex items-center justify-between py-8">
                                    <p className="font-medium text-xl leading-8 text-black">{totalItems[0]} Items</p>
                                    <p className="font-semibold text-xl leading-8 text-indigo-600">${totalItems[1]}</p>
                                </div>
                                <button
